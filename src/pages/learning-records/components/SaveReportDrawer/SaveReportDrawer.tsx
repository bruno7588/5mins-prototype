...
/** A selected filter shown as a compact chip; clicking it opens a value picker. */
function FilterChip({
  entry,
  onChange,
  onRemove,
  ...
  const meta = FILTER_BY_ID[entry.id]
  if (!meta) return null
  const opts = filterOptions(entry.id)
  const selected = opts.find((o) => o.value === entry.value)
  const label = selected ? `${meta.title}: ${selected.label}` : meta.title

  return (
    <div className="rd-chip-wrap" ref={ref}>
      <div className="rd-chip">
        <button
          type="button"
          className="rd-chip-main"
      ...
      {open && (
        <ul className="dropdown-menu rd-chip-menu" role="listbox">
          {opts.map((o) => {
            const isSel = o.value === entry.value
            return (
              <li key={o.value}>
                <button
                  type="button"
                  role="option"
                  aria-selected={isSel}
                  className={`dropdown-option${isSel ? ' is-selected' : ''}`}
                  onClick={() => {
                    onChange(o.value)
                    setOpen(false)
                  }}
                >
                  <span>{o.label}</span>
                  {isSel && <TickCircle size={16} color="var(--selected)" variant="Bold" />}
                </button>
              </li>
            )
          })}
...
function SaveReportDrawer({ open, onClose, onSave, initial, currentFilters }: SaveReportDrawerProps) {
  ...
  const [filtersListboxOpen, setFiltersListboxOpen] = useState(false)
  ...
  // A scheduled report needs ≥1 valid recipient with no malformed addresses.
  const recipientsOk = !scheduled || (validRecipients.length > 0 && !hasInvalidRecipient)
  const canSave = !nameMissing && recipientsOk
  ...
  // Editable filters (the hybrid: build/adjust the report's query right here)
  function addFilterEntry(id: string) {
    setFilters((prev) => (prev.some((f) => f.id === id) ? prev : [...prev, { id, value: null }]))
    setFiltersListboxOpen(false)
  }
  function setFilterValue(id: string, value: string) {
    setFilters((prev) => prev.map((f) => (f.id === id ? { ...f, value } : f)))
  }
  function removeFilterEntry(id: string) {
    setFilters((prev) => prev.filter((f) => f.id !== id))
  }

  // Step 1 primary: advance to scheduling, or save outright when not scheduled.
  ...
  function commit() {
    const report: SavedReport = {
      ...
      filters,
      scheduled,
      recipients: scheduled ? validRecipients : [],
      frequency,
  ...
  const subtitle =
    step === 1
      ...
      : 'Choose how often it’s sent and who receives it.'
        ...
        <div className="side-drawer__content">
          {step === 1 ? (
            <div className="rd-form">
              ...
              {/* Filters — editable chips + dashed Add Filter */}
                ...
                <div className="rd-chips">
                  {filters.map((f) => (
                    <FilterChip
                      ...
                      onRemove={() => removeFilterEntry(f.id)}
                    />
                  ))}
                  <div className="rd-add-filter-wrap" ref={addFilterRef}>
                    <button
                      type="button"
                      className="rd-add-filter"
                      aria-haspopup="listbox"
                      aria-expanded={filtersListboxOpen}
                      onClick={() => setFiltersListboxOpen((o) => !o)}
                    >
                      <Add size={20} color="var(--text-primary)" variant="Linear" />
                      Add Filter
                    </button>
                    <FilterListbox
                      open={filtersListboxOpen}
                      onClose={() => setFiltersListboxOpen(false)}
                      onSelect={addFilterEntry}
        ...
        <div className="side-drawer__footer">
          ...
          <div className="side-drawer__buttons">
            {step === 1 ? (
              ...
              >
                {scheduled ? 'Next' : isEditing ? 'Update Report' : 'Save Report'}
              </button>
            ) : (
              <>
                ...
                <button type="button" className="side-drawer__btn-secondary" onClick={() => setStep(1)}>
                  Back
                </button>
              </>
            )}
