...
2. Open it for the user:
   ...
   ```sh
   npx -y human-review@0.8.1 path/to/file.html
   ```
   ...
   ```sh
   npx -y human-review@0.8.1 http://localhost:3000/wiki
   ```
...
3. Wait for feedback. This command blocks until the user hits Send in the
   ...
   ```sh
   npx -y human-review@0.8.1 poll path/to/file.html
   ```
...
4. Apply what comes back, then start the next background poll. `--ack` clears
   ...
   ```sh
   npx -y human-review@0.8.1 poll path/to/file.html --ack
   ```
...
```sh
npx -y human-review@0.8.1 status path/to/file.html
```
